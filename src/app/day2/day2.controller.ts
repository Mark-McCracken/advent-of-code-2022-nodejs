import {
  Controller,
  Get,
  Post,
  UploadedFile,
  UseInterceptors,
} from '@nestjs/common';
import { FileInterceptor } from '@nestjs/platform-express';
import { ApiBody, ApiConsumes, ApiResponse, ApiTags } from '@nestjs/swagger';
import { FileUploadDto } from '../util/FileUploadDto';
import { Express } from 'express';
import { Day2Service } from './day2.service';
import { Day2Response } from './Day2Response';
import { readFile } from 'fs/promises';
import { resolve } from 'path';

@ApiTags('Day 2')
@Controller('day2')
export class Day2Controller {
  constructor(private service: Day2Service) {}

  @Get(`problem`)
  async getProblem(): Promise<string> {
    return String(await readFile(resolve(__dirname, 'problem.txt')));
  }

  @Post(`solve`)
  @ApiConsumes('multipart/form-data')
  @ApiBody({ description: 'The input for day 2', type: FileUploadDto })
  @ApiResponse({ description: 'The answer for day 2', type: Day2Response })
  @UseInterceptors(FileInterceptor('file'))
  uploadInput(@UploadedFile() file: Express.Multer.File): Day2Response {
    const fileContents = file.buffer.toString();
    const results = this.service.solve(fileContents);
    return results;
  }
}
